...
CHANNEL_LABELS = [ALL_CHANNEL_LABELS[i] for i in SELECTED_INDICES]
...
def make_output_file():
    ...
    base = f"PD_{DETECTOR_NAME}_only_{stamp}"
...
def main():
    if len(sys.argv) != 3:
        print("Usage:")
        print(f"  python3 read_pd_{DETECTOR_NAME}_only.py TOTAL_TIME_S INTEGRATION_TIME_S")
        sys.exit(1)
    ...
    print(f"Detector: {DETECTOR_NAME} only")
    ...
    with SMBus(I2C_BUS) as bus, output_file.open("w", newline="") as f:
        ...
        while True:
            ...
            all_deltas = compute_deltas(new_counts, old_counts)
            ...
            writer.writerow([f"{now_unix:.6f}", iso_utc, f"{elapsed_s:.3f}", *selected_deltas])
            f.flush()

            if row_count > 0 and row_count % HEADER_EVERY == 0:
                print_terminal_header()

            print(format_terminal_row(iso_utc, elapsed_s, selected_deltas))
